...
class FootballTrackerGUI:
    ...
    def setup_gui(self):
        ...
        # Control panel on the left side with a modern look
        self.control_frame = ttk.Frame(self.main_frame, padding="5", relief="sunken")
        self.control_frame.pack(side=tk.LEFT, fill=tk.Y)

        # Video display frame filling the remaining space
        ...
        self.player_dropdown = ttk.Combobox(self.control_frame, textvariable=self.player_var, state='readonly',
                                            width=30)
        self.player_dropdown.pack(side=tk.TOP, padx=5, pady=5)
        ...
        self.display_mode_button.pack(side=tk.TOP, padx=5, pady=5)

        self.generate_heatmap_button = ttk.Button(self.control_frame, text="🌡 Generate Heatmap",
                                                  command=self.show_heatmap_window)
        self.generate_heatmap_button.pack(side=tk.TOP, padx=5, pady=5)
        ...
        self.status_label = ttk.Label(self.control_frame, text="Select a video to begin tracking", font=('Arial', 10))
        self.status_label.pack(side=tk.BOTTOM, padx=5)

        # Video display with a border
    ...
    def create_field_background(self):
        """Create a realistic football field background image."""
        field = np.ones((self.field_height, self.field_width, 3), dtype=np.uint8) * 255

        # Draw green field
        field[:] = (34, 139, 34)  # Use a darker green for a realistic look

        # Draw white lines for the field outline
        cv2.rectangle(field, (50, 50), (self.field_width - 50, self.field_height - 50), (255, 255, 255), 2)

        # Draw the center line
        cv2.line(field, (self.field_width // 2, 50), (self.field_width // 2, self.field_height - 50), (255, 255, 255),
                 2)
        ...
        # Draw goal areas
        cv2.rectangle(field, (50, self.field_height // 2 - 100), (200, self.field_height // 2 + 100), (255, 255, 255),
                      2)
        ...
        # Draw penalty areas
        cv2.rectangle(field, (50, self.field_height // 2 - 200), (200, self.field_height // 2 + 200), (255, 255, 255),
                      2)
        cv2.rectangle(field, (self.field_width - 200, self.field_height // 2 - 200),
                      (self.field_width - 50, self.field_height // 2 + 200), (255, 255, 255), 2)

        # Draw corner arcs
        cv2.ellipse(field, (50, 50), (25, 25), 0, 0, 90, (255, 255, 255), 2)
        cv2.ellipse(field, (50, self.field_height - 50), (25, 25), 0, 270, 360, (255, 255, 255), 2)
        cv2.ellipse(field, (self.field_width - 50, 50), (25, 25), 0, 90, 180, (255, 255, 255), 2)
        cv2.ellipse(field, (self.field_width - 50, self.field_height - 50), (25, 25), 0, 180, 270, (255, 255, 255), 2)

        # Draw additional markings
        cv2.line(field, (self.field_width // 2, 50), (self.field_width // 2, self.field_height - 50), (255, 255, 255),
                 2)  # Center line
        cv2.circle(field, (self.field_width // 2, self.field_height // 2), 9, (255, 255, 255), -1)  # Center spot

        # Optionally, add some texture to the field
        for i in range(50, self.field_width - 50, 20):
            cv2.line(field, (i, 50), (i, self.field_height - 50), (200, 200, 200), 1)
